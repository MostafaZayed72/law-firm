<template>
    <div class="card rtl">
      <DataTable v-model:filters="filters" @click:row="editCase" :value="filteredCustomers" paginator :rows="10" dataKey="id"
        ...
        <Column field="court" header="المركز" :filter="true" :filterPlaceholder="'ابحث بالمركز'"
          style="min-width: 7rem">
          <template #body="{ data }">
            {{ data.court }}
          </template>
          <template #filter="{ filterModel, filterCallback }">
            <InputText v-model="filterModel.value" type="text" @input="filterCallback()" placeholder="ابحث بالمركز" />
          </template>
        </Column>
        ...
        <Column field="registration_date" header="الجلسة السابقة" :filter="true"
        ...
        <Column field="next_court_session" header="الجلسة القادمة" :filter="true"
        ...
        <v-text-field reverse v-model="dialog.case.defendents" label="المدعى عليه" /> -->
              ...
              <v-text-field reverse v-model="dialog.case.court" label="المركز" />
...
const clearFilters = () => {
  ...
  const fetchCases = async () => {
    try {
      ...
      customers.value = response.data.data
        ...
        .filter(item => item.case_type_relation == 5)
        .sort((a, b) => a.id - b.id);
  ...
  if (table) {
    ...
    const headers = ['عنوان القضية', 'رقم القضية', 'نوع القضية', 'درجة القضية', 'المركز', 'المدعي', 'المدعي عليه', 'موكلي', 'نوع الإعلان', 'تاريخ الجلسة السابقة', 'تاريخ الجلسة القادمة', 'الرول', 'رابط الدعوة', 'القرار', 'ملاحظات'];
    ...
    rows.forEach(row => {
      ...
      for (let i = 0; i < rowCount; i++) {
        ...
        dataIndices.forEach(index => {
          ...
          if (index === 9) { // Adjusted index for 'مدعي'
            td.textContent = i < plaintiffNames.length ? plaintiffNames[i] : '';
          } else if (index === 10) { // Adjusted index for 'مدعي عليه'
    ...
    rows: [
      new TableRow({
        children: [
          ...
          new TableCell({ children: [new Paragraph("المركز")], width: { size: 10, type: WidthType.PERCENTAGE } }),
