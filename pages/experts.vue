    ...
    <div class="card rtl">
      ...
      @page="onPageChange" :rows="10" dataKey="id" filterDisplay="row"
        ...
        <template #header>
          ...
          <h1 class="mx-auto flex justify-center items-center bg-blue-400 px-6 rounded-lg cursor-default text-white " > الخبراء</h1>
...
const fetchCases = async (page = 1, size = 25) => {
  ...
  if (table) {
    ...
    const headers = ['عنوان القضية', 'رقم القضية', 'نوع القضية', 'درجة القضية', 'الخبير المختص', 'المدعي', 'المدعي عليه', 'موكلي', 'نوع الإعلان', 'تاريخ الجلسة السابقة', 'تاريخ الجلسة القادمة', 'الرول', 'رابط الدعوة', 'القرار', 'ملاحظات'];
