<template>
    <div class="card rtl">
      <DataTable v-model:filters="filters"  @click:row="editCase" :value="filteredCustomers" paginator :rows="10" dataKey="id" filterDisplay="row"
        ...
        <template #header>
          ...
          <h1 class="mx-auto flex justify-center items-center bg-blue-400 px-6 rounded-lg cursor-default text-white " > التنفيذات</h1>
  ...
  <select style="border:1px solid grey" class="px-4 rounded" v-model="filterModel.value" @change="filterCallback()">
    ...
    <option value="false">عادية</option>
        ...
        <Column field="claimants" header="المنفذ" :filter="true" :filterPlaceholder="'ابحث بالمنفذ'"
        ...
        <Column field="defendents" header="المنفذ ضده" :filter="true" :filterPlaceholder="'ابحث بالمنفذ ضده'"
        style="min-width: 8rem">
        <template #body="{ data }">
            {{ data.defendents }}
          </template>
        <template #filter="{ filterModel, filterCallback }">
          <InputText v-model="filterModel.value" type="text" @input="filterCallback()" placeholder="ابحث بالمنفذ ضده" />
        </template>
      </Column>
   ...
   <v-dialog v-model="dialog.visible" max-width="600px" style="direction:rtl">
    <v-card>
      ...
      <v-card-subtitle>
        ...
        <v-text-field reverse v-model="dialog.case.claimants" label="المنفذ" />
        <v-text-field reverse v-model="dialog.case.defendents" label="المنفذ ضده" />
...
const clearFilters = () => {
  ...
  if (table) {
    ...
    const headers = ['عنوان القضية', 'رقم القضية', 'نوع القضية', 'درجة القضية', 'المحكمة', 'المنفذ', 'المنفذ ضده', 'موكلي', 'نوع الإعلان', 'تاريخ الجلسة السابقة', 'تاريخ الجلسة القادمة', 'الرول', 'رابط الدعوة', 'القرار', 'ملاحظات'];
