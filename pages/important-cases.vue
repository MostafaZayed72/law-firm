    ...
    <DataTable v-model:filters="filters"  @click:row="editCase"  :totalRecords="totalRecords" :value="filteredCustomers"  :paginator="true"
      ...
      <template #header>
        <div class="flex justify-start">
          ...
          <h1 class="mx-auto flex justify-center items-center bg-blue-400 px-6 rounded-lg cursor-default text-white " > القضايا الهامة</h1>
...
const fetchCases = async (page = 1, size = 25) => {
  try {
    ...
    const formatDate = (dateString) => {
      ...
      return date.toISOString().split('T')[0]; // استخراج التاريخ فقط من الـ ISO String
    };
...
const printTable = () => {
  ...
  if (table) {
    ...
    const headers = ['عنوان القضية', 'رقم القضية', 'نوع القضية', 'درجة القضية', 'المحكمة', 'المدعي', 'المدعي عليه', 'موكلي', 'نوع الإعلان', 'تاريخ الجلسة السابقة', 'تاريخ الجلسة القادمة', 'الرول', 'رابط الدعوة', 'القرار', 'ملاحظات'];
