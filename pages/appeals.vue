<template>
    <div class="card rtl">
      <DataTable v-model:filters="filters" @click:row="editCase" :value="customers" paginator :rows="10" dataKey="id"
        ...
        <v-text-field reverse v-model="dialog.case.defendents" label="المدعى عليه" /> -->
              ...
              <v-text-field reverse v-model="dialog.case.registration_date" label="تاريخ الحكم" type="date" />
              <v-text-field reverse v-model="dialog.case.next_court_session" label="آخر موعد للطعن" type="date" />
  ...
  const printTable = () => {
    ...
    if (table) {
      ...
      const headers = ['عنوان القضية', 'رقم القضية', 'نوع القضية', 'درجة القضية', 'المحكمة', 'المدعي', 'المدعي عليه', 'موكلي', 'نوع الإعلان', 'تارريخ تاريخ الحكم', 'تاريخ آخر موعد للطعن', 'الرول', 'رابط الدعوة', 'القرار', 'ملاحظات'];
      ...
      rows: [
        new TableRow({
          children: [
            ...
            new TableCell({ children: [new Paragraph("تاريخ آخر موعد للطعن")], width: { size: 10, type: WidthType.PERCENTAGE } }),
            new TableCell({ children: [new Paragraph("تاريخ تاريخ الحكم")], width: { size: 10, type: WidthType.PERCENTAGE } }),
